...
// ============================================================
// WebSocket 连接（直接 query 参数鉴权，无需 STS）
// ============================================================
...
function buildWssUrl() {
    ...
    // 把 Python 代码里的 HTTP Header 全部转为 URL query 参数
    // 浏览器 WebSocket 不支持自定义 Header，但火山引擎支持 query 参数鉴权
    const params = new URLSearchParams();
    params.set('api_app_id', appId);
    params.set('api_access_key', accessToken);
    params.set('api_resource_id', resourceId);
    params.set('api_app_key', 'PlgvMymc7f3tQnJ6');
    params.set('api_connect_id', crypto.randomUUID ? crypto.randomUUID() : uuidv4Fallback());
...
async function connectWss() {
    ...
    console.log('[VolcTTS] 正在连接 WebSocket (query 参数鉴权)...');
    ...
    return new Promise((resolve, reject) => {
        ...
        socket.onclose = (evt) => {
            if (evt.code === 1006 || evt.code === 4000) {
                console.error('[VolcTTS] WebSocket 鉴权失败，请检查 APP_ID 和 ACCESS_TOKEN 是否正确');
            }
            console.log('[VolcTTS] WebSocket 已关闭, code:', evt.code, 'reason:', evt.reason);
            ws = null;
            isConnected = false;
