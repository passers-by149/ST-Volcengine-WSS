...
async function synthesizeText(text) {
    ...
    socket.onmessage = (evt) => {
        const parsed = parseServerFrame(evt.data);
        if (parsed.event === EVENT_TTS_RESPONSE && parsed.payload instanceof Uint8Array) chunks.push(parsed.payload);
        else if (parsed.event === EVENT_TTS_ENDED) finished = true;
        else if (parsed.event === EVENT_DIALOG_COMMON_ERROR || parsed.event === EVENT_SESSION_FAILED) finished = true;
    };
    try {
...
async function speakSentences(sentences) {
    for (const s of sentences) {
        if (!s.trim()) continue;
        try { const pcm = await synthesizeText(s); if (pcm && pcm.byteLength) playPCM(pcm, settings.sample_rate); } catch (err) { console.error('[VolcTTS] 逐句失败:', err); }
    }
}
