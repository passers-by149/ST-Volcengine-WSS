...
// ============================================================
// WebSocket 连接（通过本地代理 → 火山引擎）
// 代理脚本 volcengine-proxy.js 负责添加 HTTP Header 鉴权
// ============================================================
...
async function connectWss() {
    ...
    console.log('[VolcTTS] 正在通过本地代理连接火山引擎...');
    ...
    return new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
            socket.close();
            reject(new Error('代理连接超时（15秒），请确认代理已启动'));
        }, 15000);

        let authSent = false;

        socket.onopen = () => {
            console.log('[VolcTTS] 已连接本地代理，发送鉴权信息...');
            // 连接后立即发送鉴权 JSON
        ...
        socket.onmessage = (evt) => {
            // 第一条回复是代理的 ready/error 消息
            if (typeof evt.data === 'string') {
                try {
                    const msg = JSON.parse(evt.data);
                    if (msg.type === 'ready') {
                        clearTimeout(timeout);
                        console.log('[VolcTTS] 代理已连接火山引擎，开始通信');
                        ws = socket;
                        isConnected = true;
                        resolve(socket);
                        return;
                        ...
                        clearTimeout(timeout);
                        socket.close();
                        reject(new Error('代理返回错误: ' + msg.message));
                        return;
                    }
                } catch (e) { /* 不是 JSON，继续 */ }
            }
        ...
        socket.onerror = (err) => {
            clearTimeout(timeout);
            console.error('[VolcTTS] 代理连接错误:', err);
            reject(new Error('无法连接本地代理 ws://127.0.0.1:8700，请确认 volcengine-proxy.js 已启动'));
        };

        socket.onclose = (evt) => {
            if (!authSent) {
                clearTimeout(timeout);
                reject(new Error('代理连接被拒绝，请确认 volcengine-proxy.js 已启动'));
            }
            console.log('[VolcTTS] 代理连接关闭, code:', evt.code);
            ws = null;
            isConnected = false;
...
async function setupEventListeners() {
    // 获取 ST 事件系统
    // 使用绝对路径 /script.js，避免第三方扩展相对路径解析问题
    ...
    try {
        ...
        console.warn('[VolcTTS] 绝对路径导入失败，尝试相对路径:', e.message);
        try {
            const scriptModule = await import('../../../../script.js');
            eventSource = scriptModule.eventSource;
            event_types = scriptModule.event_types;
        } catch (e2) {
            console.warn('[VolcTTS] 事件系统不可用，使用轮询模式:', e2.message);
