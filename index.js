...
async function fetchJwtToken() {
    ...
    // 通过 SillyTavern 内置 CORS 代理绕过浏览器跨域限制
    // 需要在 config.yaml 中设置 enableCorsProxy: true
    const stsTargetUrl = 'https://openspeech.bytedance.com/api/v1/sts/token';
    const proxyUrl = `/proxy/${stsTargetUrl}`;
    ...
    try {
        ...
        throw new Error(
            'STS 请求失败: ' + e.message +
            '。请确保 SillyTavern 启用了 CORS 代理：' +
            '在 config.yaml 中设置 enableCorsProxy: true，' +
            '或启动时添加 --corsProxy 参数，然后重启 SillyTavern。'
        );
    }

    if (resp.status === 404) {
        throw new Error(
            'SillyTavern CORS 代理未启用。' +
            '请在 config.yaml 中设置 enableCorsProxy: true，' +
            '或启动时添加 --corsProxy 参数，然后重启 SillyTavern。'
        );
    }
